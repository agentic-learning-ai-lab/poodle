...
class RandomResizedCropTruncNormal(torch.nn.Module):
    def __init__(
        self,
        size,
        # scale=(0.08, 1.0),
    ...
    @staticmethod
    def get_params(img: Tensor, scale_mean: float, scale_std: float, ratio: List[float]) -> Tuple[int, int, int, int]:
        ...
        for _ in range(10):
            target_area = area * torch.nn.init._no_grad_trunc_normal_(torch.empty(1), mean=scale_mean, std=scale_std, a=0.05, b=0.95).item()
    ...
    def forward(self, img):
        ...
        """
        i, j, h, w = self.get_params(img, self.scale_mean, self.scale_std, self.ratio)
        return tvF.resized_crop(img, i, j, h, w, self.size, self.interpolation, antialias=self.antialias)
...
@register_dataset
def bdd100k_rrc_truncnormal(data_dir, meta_info_file, split,
                            rrc_scale_mean, rrc_scale_std,
                            rrc_ratio=[1., 1.],
                            img_size=None,
    ...
    rrc_crop = RandomResizedCropTruncNormal(size=img_size, scale_mean=rrc_scale_mean, scale_std=rrc_scale_std, ratio=rrc_ratio, interpolation=InterpolationMode.BILINEAR, antialias=None)
    ...
    dataset = BDD100KDatasetTruncNormal(
        data_dir=data_dir,
        aug_fn=aug_fn,
        mode=split,
...
class BDD100KDatasetTruncNormal(Dataset):
    def __init__(self,
                 data_dir: str,
                 aug_fn: Callable,
                 mode: str = "train",
